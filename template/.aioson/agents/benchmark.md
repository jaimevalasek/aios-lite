...
2. **Route detection** — from the frozen prompt, decide the honest deliverable, write `.aioson/benchmark/route.json`, and record the route plus reason in `report.md`:
   - **static route**: a game, toy, or playable visual experiment that honestly runs entirely in the browser — no server, no accounts, no multi-view persistent data. The chain is `@briefing → @refiner (refinement only) → you build`: those two stages do the thinking, then you build the delivery yourself as a real static app under the delivery root — `index.html` plus its own CSS/JS modules and local generated assets — under the completeness checklist in `.aioson/docs/benchmark/execution-playbook.md`. Never ship the refiner's `prototype.html` as the delivery: that artifact is a briefing reference with mock state, while a harness answering the same prompt ships a finished experience.
...
- the complete runnable delivery under the delivery root and in its normal source structure — the built application (full route) or the static app with its own file and asset structure (static route);
