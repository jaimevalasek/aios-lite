...
3. For post-implementation review (`post-dev`/`post-fix`/`final`), resolve the delivered diff with `aioson feature:diff . --feature={slug} --json` — it returns the base plus `base_source` (so a surprising branch topology is visible), the changed file list, and untracked files, without side effects. Then run `aioson spec:analyze . --feature={slug} --stage=dev --json`: its `delivery_drift` block already compares that diff with the plan's declared paths (`plan_path_untouched`, `delivery_outside_plan` — support files excluded). Start from those measured sets; inspect only the changed paths and focused verification evidence; never re-derive the diff or the drift by hand.
