---
description: Historical full Codex AGENTS entrypoint preserved for compatibility archaeology
task_types: [legacy-gateway-reference]
load_tier: archive
---

# Legacy AIOSON AGENTS Entrypoint

> **NON-EXECUTABLE HISTORY:** Do not load this file during normal routing. The compact gateway kernel and routed modules supersede it. Consult it only to diagnose a legacy behavior or migration regression.
...
- Briefing refinement with `@refiner` to review, annotate, and refine an existing briefing before PRD generation.
...
| @refiner | "refine this briefing", "review the briefing before PRD", "create an editable briefing review", "apply briefing refinement feedback" |
...
- Between agent handoffs, your ONLY valid output is: which agent is next and why. Do not continue into that agent's work. Single exception: when the current activation explicitly includes `--auto`, `auto_handoff: true` is set in `.aioson/context/project.context.md`, or a seeded `.aioson/context/workflow-execute.json` has `agentic_policy.enabled`, the agents covered by `.aioson/docs/autopilot-handoff.md` auto-invoke the next skill. An explicit `--step` disables Autopilot for that activation and wins if both flags are present; activation flags do not rewrite the persisted preference. The chain is `@product → @planner → @dev → @qa`; Sheldon is optional. DEV may dispatch explicitly enabled development lanes, remains their integration owner, and pauses on unavailable host/model unless the manifest declares an applicable fallback. Tester, Pentester, and Validator are disabled by default and run only when explicitly enabled and triggered. The chain stops for genuine human decisions and never auto-runs `feature:close`/publish.
...
- @refiner → `.aioson/agents/refiner.md`
...
For every tracked MICRO/SMALL/MEDIUM feature, load `.aioson/docs/feature-completeness-contract.md`. Close the trace `CAP -> current-system fit -> AC -> implementation delta -> vertical phase -> exact files -> executable check -> production-path evidence`. Requirements/spec/design/readiness/conformance/harness artifacts are not canonical prerequisites. Simple Plan remains the separate lane for an already-specified bounded technical outcome.
...
Agents that load it: @briefing, @refiner, @product, @sheldon, @analyst, @architect, @scope-check, @qa
...
Agents that load them: @briefing, @refiner, @product, @sheldon
