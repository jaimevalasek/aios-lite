---
description: Historical Neo routing snapshot retained for compatibility archaeology
agents: [neo]
task_types: [legacy-routing-audit]
triggers: [legacy neo behavior, removed routing rule, compatibility investigation]
---

# Legacy Neo Routing Reference

> Historical snapshot only. This file is non-executable and must not be loaded during ordinary Neo activation. The active contract lives in `.aioson/agents/neo.md`.
