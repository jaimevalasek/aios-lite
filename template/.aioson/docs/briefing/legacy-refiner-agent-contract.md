...
2. Resolve the visual route from `design_skill` in `.aioson/context/project.context.md`:
   ...
   - **a project-forged skill:** if `design_skill` names a skill this project forged (site-forge or hybrid output), use it. If `design_skill` is blank for a `site`/`web_app` and the user does not want the reference-image route, `interface-design` runs intent-first — the framework ships no fixed presets to pick from.
