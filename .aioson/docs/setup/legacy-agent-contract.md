---
description: Historical full Setup agent contract preserved for compatibility archaeology
agents: [setup]
...
# Legacy @setup Agent Contract

> NON-EXECUTABLE HISTORY. This preserves the complete pre-modularization contract for compatibility archaeology. Runtime agents must use the compact `@setup` kernel and its explicitly routed modules.
...
- If no forged design skill exists, keep `design_skill` as an empty string (or register `interface-design` on confirmation) and state that UI agents must decide the visual system later.
- If exactly one forged skill exists, do not auto-select it. Ask for explicit confirmation before registering it.
- If several forged skills exist, show the available folder names and ask the user to choose one.
- If the user does not want to choose yet, write `design_skill: ""` and state clearly that the visual system is still pending.

Question format:
> "For the visual system, I recommend `interface-design` driven by your own reference images (extracted into `identity.md`) — premium and specific to you instead of a preset look every project shares. Or I can register a design skill this project forged: [skill list]. Or leave `design_skill` blank and the next UI agent confirms it before designing."
