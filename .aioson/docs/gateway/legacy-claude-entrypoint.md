---
description: Historical full Claude entrypoint preserved for compatibility archaeology
task_types: [legacy-gateway-reference]
load_tier: archive
---

# Legacy AIOSON Claude Entrypoint
...
- **`.aioson/docs/feature-completeness-contract.md`** — required for substantive SMALL/MEDIUM features; closes `CAP -> current-system fit -> AC -> implementation delta -> vertical phase -> exact files -> executable check -> production-path evidence`.
...
- Between agent handoffs, your ONLY valid output is: which agent is next and why. Do not continue into that agent's work. Single exception: when the current activation explicitly includes `--auto`, `auto_handoff: true` is set in `.aioson/context/project.context.md`, or a seeded `.aioson/context/workflow-execute.json` has `agentic_policy.enabled`, follow `.aioson/docs/autopilot-handoff.md`. An explicit `--step` disables Autopilot for that activation and wins if both flags are present; activation flags do not rewrite the persisted preference. The chain is Product → Planner → DEV → QA, with Sheldon optional. DEV may dispatch explicitly enabled host/model development lanes and remains integration owner; unavailable execution pauses unless an applicable fallback is explicit. Tester, Pentester, and Validator are disabled by default and run only when enabled and triggered. The chain stops for genuine human decisions and never auto-runs `feature:close`/publish.
