...
2. **Route detection** — from the frozen prompt, decide the honest deliverable and record the route plus reason in `report.md`:
   - **prototype route**: a game, toy, or playable visual experiment that lives in one self-contained screen (no server, no accounts, no multi-view persistent data). The chain is `@briefing → @briefing-refiner`, and the refiner's full working `prototype.html` is the deliverable — the comparable artifact to a harness's single HTML.
   - **full route**: everything else — site, CRM, dashboard, SaaS, API, any real project a developer would build with Node.js, React, Vite, or equivalent. The chain is `@briefing → @briefing-refiner (no prototype) → @product → @sheldon → @planner → @dev → @qa`, producing running software. When in doubt, take the full route.
