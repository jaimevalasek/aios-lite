...
module.exports = {
  briefing_gate: {
    ...
    status_invalid_fix: '  Status acceptés : draft ou approved. Corrigez {path}.',
    skipped_measured_run: 'Le briefing "{slug}" appartient à un run de benchmark mesuré : l\'approbation y est dispensée (prototype_skipped_measured_run).',
    skipped_measured_run_fix: '  Une traversée mesurée ne devient jamais une autorité produit. Supprimez .aioson/benchmark/measured-run.json seulement si ce workspace est un vrai projet.',
    craft_below_bar: 'Le briefing "{slug}" ne peut pas être approuvé : le prototype mesure sous la barre premium (prototype_visual_craft_below_bar).',
    craft_below_bar_detail: '  {detail}',
    craft_below_bar_fix: '  Reconstruisez à partir des références du propriétaire (reference-identity-extract) ou à la barre premium du registre, remesurez avec verify:artifact --kind=visual --runtime puis approuvez à nouveau — ou enregistrez la décision avec --accept-craft.',
    generic: 'Impossible d\'approuver le prototype de "{slug}" : {error}.'
  },
  cli: {
    title: 'AIOSON CLI',
    title_line: '{title}\n',
    ...
    help_update:
      'aioson update [path] [--dry-run] [--allow-downgrade] [--lang=<bcp47-tag>] [--locale=fr]',
    ...
    rule_new: {
      ...
      name_required: 'rule:new exige --name=<nom-kebab-case>.',
      invalid_name: 'Le nom de regle "{name}" doit etre en kebab-case (lettres, chiffres, traits d union simples).',
      not_project: 'Aucun repertoire .aioson/ ici - executez cette commande dans un projet AIOSON.',
      exists: 'La regle "{name}" existe deja dans {path}. Modifiez-la, ou utilisez --force pour la remplacer.',
      invalid_load_tier: 'load_tier doit valoir `always` ou `trigger`.',
      invalid_priority: 'priority doit etre un nombre entre 0 et 100.',
      no_routing_dimension: 'Avertissement : aucun agents, triggers, task-types ou paths declare, donc context:select atteindra rarement cette regle. Declarez-en au moins un, ou utilisez --load-tier=always.',
      next: 'Etape suivante : remplacez les enonces d exemple par des exigences concretes et verifiables, puis lancez aioson verify:artifact . --kind=rule --file={path}',
      error: 'rule:new a echoue ({reason}).'
    },
    help_briefing_sources:
      'aioson briefing:sources [chemin] [--slug=<slug>] [--json] [--locale=fr]',
    help_briefing_approve:
      'aioson briefing:approve [chemin] [--slug=<slug>] [--accept-craft] [--locale=fr]',
    ...
    briefing_sources: {
      error: 'Impossible d’inspecter les sources du Briefing ({error}) pour {slug}.',
      inspected: 'Paquet de sources "{slug}" inspecté : {files} fichier(s), SQL={sql}, bloqués={blocked}.',
      none: 'Aucun paquet de sources du Briefing trouvé dans plans/.',
      discovered: '{packs} paquet(s) de sources et {loose} fichier(s) isolé(s) trouvés.'
    },
    ...
    help_context_usage:
      'aioson context:usage [path] [--since=<jours>] [--feature=<slug>] [--json] [--locale=fr]',
    help_context_load:
    ...
    help_exploration:
      'aioson exploration:<init|configure|references|intake|scan|add-run|record|validate|status|review|select|promote|run> [chemin] --slug=<slug> [options] [--json]',
    help_chain_list:
      'aioson chain:list [chemin] [--feature=<slug>] [--agent=dev] [--status=<open,claimed>] [--include-resolved] [--limit=N] [--json] [--locale=fr]',
    help_chain_claim:
      'aioson chain:claim [chemin] --agent=<agent> [--id=NC-N] [--feature=<slug>] [--limit=N] [--lease-minutes=N] [--json] [--locale=fr]',
    help_chain_resolve:
      'aioson chain:resolve [chemin] --id=NC-N --agent=<agent> --token=<token> --outcome=<fixed|verified-no-change|false-positive|obsolete> --evidence=<texte> [--json] [--locale=fr]',
    help_chain_release:
      'aioson chain:release [chemin] --id=NC-N --agent=<agent> --token=<token> [--json] [--locale=fr]',
    help_chain_reconcile:
      'aioson chain:reconcile [chemin] [--json] [--locale=fr]',
    chain_audit: {
    ...
    help_memory_search:
      'aioson memory:search "<requête>" [path] [--limit=5] [--surface=rules|learnings|all] [--include-archived] [--json] [--locale=fr]',
    help_memory_archive:
      'aioson memory:archive [path] --id=<rule|learning|brain>:<slug> --reason="<texte>" [--feature=<slug>] [--dry-run] [--json] [--locale=fr]',
    help_memory_restore:
      'aioson memory:restore [path] --id=<rule|learning|brain>:<slug> [--reason="<texte>"] [--feature=<slug>] [--dry-run] [--json] [--locale=fr]',
    memory_archive: {
      id_required: 'memory:archive exige --id=<rule|learning|brain>:<slug>.',
      reason_required: 'memory:archive exige --reason="<texte>".',
      invalid_id: 'memory:archive valeur invalide pour --id : « {value} ». Attendu rule|learning|brain:<slug>.',
      hook_blocked: 'memory:archive ne peut pas être invoqué par un hook (BR-ALL-01 : tier-2 exige une action humaine).',
      target_not_found: 'memory:archive : {kind} « {slug} » introuvable à l’état actif.',
      already_archived: 'memory:archive : « {path} » déjà archivé. No-op.',
      notify_template: 'archivage de {kind} « {slug} » : {reason}',
      dry_run_summary: 'memory:archive [dry-run] : déplacerait {source} → {dest} (entrée active : {has_active}).',
      archived_success: 'memory:archive ✓ {kind} « {slug} » archivé vers {dest}.'
    },
    memory_restore: {
      id_required: 'memory:restore exige --id=<rule|learning|brain>:<slug>.',
      invalid_id: 'memory:restore valeur invalide pour --id : « {value} ». Attendu rule|learning|brain:<slug>.',
      hook_blocked: 'memory:restore ne peut pas être invoqué par un hook (BR-ALL-01 : tier-2 exige une action humaine).',
      target_not_archived: 'memory:restore : {kind} « {slug} » introuvable dans l’archive.',
      target_already_active: 'memory:restore : {kind} « {slug} » déjà actif. No-op.',
      target_not_found: 'memory:restore : {kind} « {slug} » introuvable.',
      notify_template: 'restauration de {kind} « {slug} » : {reason}',
    ...
    memory_search: {
      ...
      query_unparseable: 'memory:search requête « {value} » devient vide après nettoyage (uniquement opérateurs / guillemets).',
      invalid_surface: 'memory:search valeur invalide pour --surface : {value}. Attendu rules, learnings ou all.',
      no_results: 'Aucun résultat pour « {query} ».',
      results_header: 'Top {count} résultats pour « {query} » :',
    ...
    help_browser_run:
      'aioson browser:run [path] --script=<walkthrough.json> [--url=<url-app>|--file=<html>] [--slug=<feature>] [--prototype] [--cdp=<endpoint>] [--browser=chrome|msedge|chromium] [--headed] [--continue] [--out=<dir>] [--no-persist] [--json] [--locale=fr]',
    help_browser_snapshot:
      'aioson browser:snapshot [path] --url=<url-page>|--file=<html> [--target=<locator>] [--max-lines=80] [--cdp=<endpoint>] [--browser=chrome|msedge|chromium] [--headed] [--json] [--locale=fr]',
    help_qa_report:
    ...
    help_delegation_plan:
      'aioson delegation:plan [path] --explicit-model-request --host=<host> [--provider=<provider>] --model=<modèle> --kind=<type> (--task=<texte>|--task-file=<chemin>) [--json]',
    help_delegation_run:
      'aioson delegation:run [path] --explicit-model-request --host=<host> [--provider=<provider>] --model=<modèle> --kind=<type> (--task=<texte>|--task-file=<chemin>) [--json]',
    harnessRetro: {
    ...
    help_genome_approve:
      'aioson genome:approve [path] --squad=<slug> --genome=<slug> [--executor=<slug>] [--specimen=<path>] [--json] [--locale=fr]',
    help_genome_migrate:
  ...
  hooks_setup: {
    installing: 'Installation des hooks AIOSON (context:guard + telemetrie runtime) — desactivez avec --no-hooks :',
    skipped: 'Hooks ignores (--no-hooks). Installez plus tard avec : aioson hooks:install',
    failed: 'Echec de l installation des hooks (non bloquant) : {message}. Installez plus tard avec : aioson hooks:install'
  },
  init_all: {
  ...
  update: {
    ...
    downgrade_blocked: 'Ce projet a ete mis a jour pour la derniere fois avec AIOSON {projectVersion}, mais ce CLI est en {cliVersion}. Mettre a jour maintenant retrograderait les fichiers geres. Mettez d abord a jour le CLI (npm install -g @jaimevalasek/aioson) puis relancez update, ou passez --allow-downgrade pour continuer malgre tout.',
  ...
  doctor: {
    ...
    visual_runtime: 'Telemetrie visuelle runtime (Playwright present)',
    visual_runtime_hint: 'Gates visuels statiques uniquement. Activez la mesure navigateur avec : npm i -D playwright && npx playwright install chromium',
    ...
    fix_action_claude_commands: 'Restaurer les slashes manquants dans .claude/commands/aioson/agent/* depuis le template',
    fix_action_features_dir: 'Creer le repertoire .aioson/context/features/',
    fix_action_permissions_in_sync: 'Regenerer les fichiers natifs de permissions depuis autonomy-protocol.json',
    fix_action_bootstrap_coverage: 'Executez /discover pour peupler les fichiers bootstrap (semantique, non automatique)',
    fix_action_version_drift: 'Mettez a jour le CLI (npm i -g @jaimevalasek/aioson) ou ajustez project.context.md aioson_version (manuel)',
    ...
    bootstrap_coverage: 'Couverture du bootstrap : {present}/{required}',
    bootstrap_coverage_hint: 'Executez /discover pour rafraichir les fichiers de bootstrap.',
    ...
    retired_design_preset_hint_local: "Le framework ne livre plus de presets visuels fixes — chaque projet qui choisissait le meme se ressemblait. {path} est une copie locale installee par une version anterieure ; elle continue de fonctionner tant que design_skill la nomme. Route recommandee : design_skill 'interface-design' (le moteur) avec les references du proprietaire extraites dans identity.md (.aioson/docs/reference-identity.md).",
    retired_design_preset_hint_missing: "Le framework ne livre plus de presets visuels fixes et aucune copie n'existe sous .aioson/skills/design/{id}/ ni .aioson/installed-skills/{id}/ — les producteurs visuels retombent sur le moteur interface-design. Definissez design_skill a 'interface-design' (avec identity.md issu des references du proprietaire) ou laissez-le vide volontairement.",
    retired_design_preset_profile: 'Le profil d installation enregistre selectionne encore des presets visuels retires ({ids}) — rien ne les livre ; le moteur interface-design est installe pour tous les profils. `aioson install . --reconfigure` reecrit le profil sans la question de design.',
    ...
    version_drift_hint: 'project.context.md aioson_version ({context}) differe du CLI ({cli}). Mettez a jour manuellement.',
    permissions_in_sync: 'Permissions natives synchronisees avec autonomy-protocol.json ({drifted} drift, {missing} absents)',
    permissions_in_sync_hint: 'Executez `aioson doctor . --fix` pour regenerer : {paths}.',
    permissions_protocol_missing_hint: 'autonomy-protocol.json absent — executez `aioson update .` pour reinstaller.',
    learning_loop: {
      distillation_complete: 'distillation : {promoted} promus, {review} à réviser, {merge} candidats à merge ({duration}ms)',
      distillation_failed_silent: 'distillation a échoué silencieusement pour feature « {slug} » — phase : {phase}',
      skipped_micro: 'distillation ignorée : classification feature MICRO',
      skipped_no_distill: 'distillation ignorée : drapeau --no-distill actif',
      lock_held: 'distillation ignorée : autre instance en cours pour « {slug} »',
      notify_template: 'distillation : {promoted} promus, {review} à réviser, {merge} candidats à merge'
    },
    living_memory: {
      rule_staleness: 'Rules obsolètes : {stale} sur {total} sans chargement durant les {threshold} dernières features fermées',
      rule_staleness_hint: 'Rules obsolètes (5 premières) : {slugs}. Proposition : {propose}',
      rule_staleness_skipped_micro: 'Vérification rule_staleness ignorée : classification du projet est MICRO (BR-ALL-11)',
      learning_orphans: 'Learnings orphelins : {orphans} learnings promus en rules jamais chargées après promotion',
      learning_orphans_hint: 'learning_ids orphelins (5 premiers) : {ids}. Utilisez `aioson memory:why --id=<id>` pour inspecter.',
      learning_orphans_skipped_micro: 'Vérification learning_orphans ignorée : classification du projet est MICRO (BR-ALL-11)',
  ...
  qa_doctor: {
    ...
    browser_cdp_ok: 'Endpoint CDP joignable ({endpoint}) : {browser}.',
    browser_cdp_unreachable: 'Endpoint CDP injoignable ({endpoint}).',
    browser_cdp_hint: 'Lancez le navigateur avec --remote-debugging-port=9222 (avec un --user-data-dir dédié) ou activez-le dans chrome://inspect/#remote-debugging.',
    browser_chrome_ok: 'Google Chrome installé ({binary}) - utilisable avec --browser=chrome.',
    browser_chrome_missing: 'Google Chrome non détecté.',
    browser_edge_ok: 'Microsoft Edge installé ({binary}) - utilisable avec --browser=msedge.',
    browser_edge_missing: 'Microsoft Edge non détecté.',
    browser_preferred: 'Navigateur par défaut pour qa:run / browser:run : {label}.',
    browser_none: 'Aucun navigateur disponible : pas de Chromium embarqué, pas de Chrome/Edge installé, pas d\'endpoint CDP.',
    browser_none_hint: 'Installez Google Chrome, exécutez `npx playwright install chromium` ou attachez-vous avec --cdp=http://127.0.0.1:9222.',
    chromium_missing_fallback: 'Chromium embarqué introuvable - utilisation de {label}.'
  },
  browser_run: {
    script_missing: 'Aucun script de walkthrough fourni.',
    script_missing_hint: 'Passez --script=<walkthrough.json> (grammaire des étapes dans .aioson/docs/qa/browser-walkthrough.md).',
    script_invalid: 'Script de walkthrough invalide : {errors}',
    failed: 'Le walkthrough n\'a pas pu s\'exécuter : {error} {detail}',
    hint: '  Indice : {hint}',
    header: 'Walkthrough "{name}" -> {target} [{browser}]',
    step_ok: '  ok    #{index} {action} : {detail} ({ms}ms)',
    step_fail: '  ECHEC #{index} {action} : {error}',
    failure_snapshot: '  Page au moment de l\'échec (aria snapshot) :',
    ids_title: 'Ids prouvés :',
    id_line: '  {id} : {status} (étapes {steps})',
    warning: 'Avertissement : {warning}',
    console_summary: 'Console : {errors} erreurs, {page_errors} erreurs de page, {warnings} avertissements.',
  ...
  browser_snapshot: {
    target_missing: 'Aucune page fournie.',
    target_missing_hint: 'Passez --url=<url-page> ou --file=<html>.',
    failed: 'Le snapshot n\'a pas pu s\'exécuter : {error} {detail}',
    header: 'Snapshot de {url} - "{title}" [{browser}]',
    login_wall: 'Arrivé sur un écran de connexion : attachez-vous au navigateur déjà connecté avec --cdp plutôt que de scripter des identifiants.',
    empty: '(arbre d\'accessibilité vide)',
    truncated: '... {shown} lignes sur {total} affichées (augmentez --max-lines pour en voir plus).'
  },
  qa_init: {
  ...
  qa_run: {
    browser_unavailable: 'Navigateur indisponible ({error}). {hint}',
  ...
  scan_project: {
    ...
    folder_required_example_cli:
      '  Sans API LLM    : aioson scan:project . --folder=src  -> transmettez la carte a @planner avec un PRD actif',
    folder_required_example_prompt:
      '  Prompt pret     : aioson agent:prompt planner --tool=codex',
    ...
    spec_found: '  Heritage : spec.md trouve — contexte historique facultatif, sans autorite de livraison',
    ...
    architecture_note: '  Note     : les artefacts du scan sont des preuves du depot ; ils n ajoutent aucune etape ni document obligatoire.',
    ...
    local_cli_step_analyst: '  2. Avec un PRD actif, lancez @planner — il utilise les cartes du scan comme preuve pour le plan unique',
    local_cli_step_prompt_codex: '  3. Si le client ne comprend pas @planner, generez un prompt pret : aioson agent:prompt planner --tool=codex',
    ...
    local_workflow_title: '\n\x1b[33m  Apres la carte du depot :\x1b[0m',
    local_step_architect: '  Ensuite : lancez @planner avec le PRD actif — il ecrit le plan executable unique',
    local_step_dev: '  Puis : lancez @dev et @qa — les specialistes optionnels repondent a une question concrete',
    next_steps: '\n  Etapes suivantes :',
    step_analyst: '  1. Avec un PRD actif, lancez @planner — utilisez discovery.md + skeleton-system.md comme preuve pour un plan',
    step_architect: '  2. Lancez @dev — implementez le plan par le point d entree normal de production',
    step_dev: '  3. Lancez @qa — effectuez une revue proportionnee et un smoke sur le chemin reel'
  },
  squad_investigate: {
  ...
  squadEval: {
    ...
    invalid_slug: 'Slug de squad invalide : {slug}.',
    manifest_missing: 'Manifeste du squad introuvable : {slug}.',
    result: 'Évaluation du squad {slug} : {verdict}',
    report: 'Rapport : {path}'
  },
