...
module.exports = {
  briefing_gate: {
    resolution_missing: 'El briefing "{slug}" no puede aprobarse: prototipo sin resolver (prototype_resolution_missing).',
    resolution_missing_expected: '  Esperado: {path}',
    resolution_missing_visual: '  Alcance visual → activa @refiner para generar el prototipo antes de la aprobación.',
    resolution_missing_non_visual: '  Feature sin superficie visual → registra la línea `prototype: not_applicable` en briefings.md y aprueba de nuevo.',
    manifest_missing: 'El briefing "{slug}" tiene prototype.html, pero falta el manifiesto (prototype_manifest_missing).',
    manifest_missing_expected: '  Esperado: {path} con `feature: {slug}` y `status: draft` — @refiner debe generarlo.',
    owner_mismatch: 'El manifiesto del prototipo pertenece a otra feature ("{owner}", se esperaba "{slug}") (prototype_manifest_owner_mismatch).',
    owner_mismatch_fix: '  Corrige el campo `feature:` en {path} — nunca reutilices el prototipo de otro briefing.',
    status_invalid: 'El manifiesto del prototipo tiene un status inválido "{status}" (prototype_manifest_status_invalid).',
    status_invalid_fix: '  Status aceptados: draft o approved. Corrige {path}.',
    skipped_measured_run: 'El briefing "{slug}" pertenece a una ronda medida de benchmark: la aprobación se dispensa allí (prototype_skipped_measured_run).',
    skipped_measured_run_fix: '  Una travesía medida nunca se convierte en autoridad de producto. Elimina .aioson/benchmark/measured-run.json solo si este workspace es un proyecto real.',
    craft_below_bar: 'El briefing "{slug}" no puede aprobarse: el prototipo mide por debajo del listón premium (prototype_visual_craft_below_bar).',
    craft_below_bar_detail: '  {detail}',
    craft_below_bar_fix: '  Reconstruye a partir de las referencias del propietario (reference-identity-extract) o al listón premium del registro, vuelve a medir con verify:artifact --kind=visual --runtime y aprueba de nuevo — o registra la decisión con --accept-craft.',
    generic: 'No se pudo aprobar el prototipo de "{slug}": {error}.'
  },
  cli: {
    title: 'AIOSON CLI',
    title_line: '{title}\n',
    ...
    help_briefing_approve:
      'aioson briefing:approve [ruta] [--slug=<slug>] [--accept-craft] [--locale=es]',
    help_briefing_unapprove:
      'aioson briefing:unapprove [ruta] [--slug=<slug>] [--locale=es]',
    ...
    help_context_select:
      'aioson context:select [path] [--explain=<path[,path2]>] [--agent=<agente>] [--mode=planning|executing] [--task=<texto>] [--paths=<ruta[,ruta2]>] [--feature=<slug>] [--no-semantic] [--json] [--locale=es]',
    help_context_brief:
    ...
    help_context_evals:
      'aioson context:evals [path] [--strict] [--filter=<texto>] [--no-coverage] [--json] [--locale=es]',
    help_context_usage:
      'aioson context:usage [path] [--since=<dias>] [--feature=<slug>] [--json] [--locale=es]',
    help_context_load:
      'aioson context:load [path] --target=<rule|brain|doc|skill>:<slug> --agent=<nombre> [--batch="slug1,slug2"] [--feature=<slug>] [--classification=<MICRO|SMALL|MEDIUM>] [--verbose] [--json] [--locale=es]',
    help_chain_audit:
      'aioson chain:audit <archivo> [path] [--limit=N] [--feature=<slug>] [--json] [--locale=es]',
    help_exploration:
      'aioson exploration:<init|configure|references|intake|scan|add-run|record|validate|status|review|select|promote|run> [ruta] --slug=<slug> [opciones] [--json]',
    help_chain_list:
      'aioson chain:list [ruta] [--feature=<slug>] [--agent=dev] [--status=<open,claimed>] [--include-resolved] [--limit=N] [--json] [--locale=es]',
    help_chain_claim:
      'aioson chain:claim [ruta] --agent=<agente> [--id=NC-N] [--feature=<slug>] [--limit=N] [--lease-minutes=N] [--json] [--locale=es]',
    help_chain_resolve:
      'aioson chain:resolve [ruta] --id=NC-N --agent=<agente> --token=<token> --outcome=<fixed|verified-no-change|false-positive|obsolete> --evidence=<texto> [--json] [--locale=es]',
    help_chain_release:
      'aioson chain:release [ruta] --id=NC-N --agent=<agente> --token=<token> [--json] [--locale=es]',
    help_chain_reconcile:
      'aioson chain:reconcile [ruta] [--json] [--locale=es]',
    chain_audit: {
    ...
    context_load: {
      target_required: 'context:load requiere --target=<rule|brain|doc|skill>:<slug>.',
    ...
    help_locale_diff: 'aioson locale:diff [agent] [--lang=<bcp47-tag>] [--json] [--locale=es]',
    help_test_agents: 'aioson test:agents [ruta] [--json] [--locale=es]',
    help_test_smoke:
    ...
    help_review_prepare:
      'aioson review:prepare [ruta] --agent=<agente> --feature=<slug> [--artifact=<ruta>] [--json] [--locale=es]',
    help_review_check:
      'aioson review:check [ruta] --agent=<agente> --feature=<slug> --report=<ruta> [--json] [--locale=es]',
    help_review_status:
      'aioson review:status [ruta] --feature=<slug> [--json] [--locale=es]',
    help_parallel_init:
    ...
    help_qa_run:
      'aioson qa:run [path] [--url=<app-url>] [--feature=<slug>] [--persona=naive|hacker|power|mobile] [--headed] [--html] [--json] [--locale=es]',
    help_qa_scan:
      'aioson qa:scan [path] [--url=<app-url>] [--depth=3] [--max-pages=50] [--headed] [--html] [--json] [--locale=es]',
    help_browser_run:
      'aioson browser:run [path] --script=<walkthrough.json> [--url=<url-app>|--file=<html>] [--slug=<feature>] [--prototype] [--cdp=<endpoint>] [--browser=chrome|msedge|chromium] [--headed] [--continue] [--out=<dir>] [--no-persist] [--json] [--locale=es]',
    help_browser_snapshot:
      'aioson browser:snapshot [path] --url=<url-pagina>|--file=<html> [--target=<locator>] [--max-lines=80] [--cdp=<endpoint>] [--browser=chrome|msedge|chromium] [--headed] [--json] [--locale=es]',
    help_qa_report:
    ...
    help_pentester_coverage:
      'aioson pentester:coverage [ruta] --feature=<slug> [--json] [--locale=es]',
    help_workflow_mode:
    ...
    help_feature_trace:
      'aioson feature:trace [ruta] --feature=<slug> [--json] [--locale=es]',
    help_feature_diff:
      'aioson feature:diff [ruta] --feature=<slug> [--base=<ref>] [--paths-only] [--json] [--locale=es]',
    help_setup_detect:
    ...
    help_delegation_plan:
      'aioson delegation:plan [path] --explicit-model-request --host=<host> [--provider=<provider>] --model=<modelo> --kind=<tipo> (--task=<texto>|--task-file=<ruta>) [--json]',
    help_delegation_run:
      'aioson delegation:run [path] --explicit-model-request --host=<host> [--provider=<provider>] --model=<modelo> --kind=<tipo> (--task=<texto>|--task-file=<ruta>) [--json]',
    harnessRetro: {
    ...
    help_genome_approve:
      'aioson genome:approve [path] --squad=<slug> --genome=<slug> [--executor=<slug>] [--specimen=<path>] [--json] [--locale=es]',
    help_genome_migrate:
    ...
    help_squad_validate:
      'aioson squad:validate [path] --squad=<slug> [--strict] [--json] [--locale=es]',
    help_squad_eval:
      'aioson squad:eval [path] --squad=<slug> [--json] [--locale=es]',
    help_squad_pilot_approve:
      'aioson squad:pilot-approve [path] --squad=<slug> [--json] [--locale=es]',
    help_squad_export:
  ...
  init: {
    usage_error:
      'Uso: aioson init <project-name> [--force] [--dry-run] [--all] [--no-hooks] [--lang=<bcp47-tag>] [--tool=codex|claude|opencode] [--locale=es]',
  ...
  update: {
    ...
    agent_renamed: 'i El agente `briefing-refiner` ahora se llama `refiner` (`/refiner`, `@refiner`). Se eliminaron {count} archivo(s) del nombre anterior; el id antiguo sigue resolviendo como alias.',
  ...
  doctor: {
    ...
    context_language_mismatch:
      '`interaction_language` ({interaction}) difiere de `conversation_language` ({conversation})',
    context_language_mismatch_hint:
      'Mantenga ambos campos sincronizados o elimine el alias heredado `conversation_language`.',
    node_version: 'Node.js >= 18 (actual: {version})',
    visual_runtime: 'Telemetria visual de runtime (Playwright presente)',
    visual_runtime_hint: 'Gates visuales solo estaticos. Habilita la medicion en browser con: npm i -D playwright && npx playwright install chromium',
    ...
    retired_design_preset_hint_local: "El framework ya no incluye presets visuales fijos — cada proyecto que elegia el mismo se veia igual. {path} es una copia local instalada por una version anterior; sigue funcionando mientras design_skill la nombre. Ruta recomendada: design_skill 'interface-design' (el motor) con las referencias del propietario extraidas en identity.md (.aioson/docs/reference-identity.md).",
    retired_design_preset_hint_missing: "El framework ya no incluye presets visuales fijos y no existe copia en .aioson/skills/design/{id}/ ni en .aioson/installed-skills/{id}/ — los productores visuales recurren al motor interface-design. Define design_skill como 'interface-design' (con identity.md a partir de las referencias del propietario) o dejalo vacio a proposito.",
    retired_design_preset_profile: 'El perfil de instalacion guardado todavia selecciona presets visuales retirados ({ids}) — nada los incluye; el motor interface-design se instala en todos los perfiles. `aioson install . --reconfigure` reescribe el perfil sin la pregunta de diseno.',
  ...
  locale_apply: {
    ...
    context_updated: 'Campos de idioma de project.context.md sincronizados: {locale}',
    context_would_update: '[dry-run] Los campos de idioma de project.context.md se sincronizarian: {locale}',
    context_unchanged: 'Los campos de idioma de project.context.md ya estaban sincronizados: {locale}',
    context_missing: 'No se encontro project.context.md; la preferencia de idioma no se guardo.',
    context_invalid: 'project.context.md tiene frontmatter invalido; la preferencia de idioma no cambio.'
  },
  smoke: {
  ...
  workflow_plan: {
    ...
    note_product_optional:
      'Para trabajo tecnico acotado y ya especificado, usa Simple Plan; las features MICRO rastreadas usan Product → Sheldon → Planner → Dev → QA.',
    note_feature_flow:
      'Flujo para nueva feature (tras la configuracion inicial): @product → @sheldon → @planner → @dev → @qa. Sheldon realiza la revision independiente obligatoria del PRD. Un PRD, un plan y un veredicto de QA.'
  },
  parallel_init: {
    context_missing:
  ...
  qa_doctor: {
    ...
    browser_cdp_ok: 'Endpoint CDP alcanzable ({endpoint}): {browser}.',
    browser_cdp_unreachable: 'Endpoint CDP no alcanzable ({endpoint}).',
    browser_cdp_hint: 'Inicia el navegador con --remote-debugging-port=9222 (con --user-data-dir dedicado) o actívalo en chrome://inspect/#remote-debugging.',
    browser_chrome_ok: 'Google Chrome instalado ({binary}) - utilizable con --browser=chrome.',
    browser_chrome_missing: 'Google Chrome no detectado.',
    browser_edge_ok: 'Microsoft Edge instalado ({binary}) - utilizable con --browser=msedge.',
    browser_edge_missing: 'Microsoft Edge no detectado.',
    browser_preferred: 'Navegador por defecto para qa:run / browser:run: {label}.',
    browser_none: 'Ningún navegador disponible: sin Chromium empaquetado, sin Chrome/Edge instalado, sin endpoint CDP.',
    browser_none_hint: 'Instala Google Chrome, ejecuta `npx playwright install chromium` o conecta con --cdp=http://127.0.0.1:9222.',
    chromium_missing_fallback: 'Chromium empaquetado no encontrado - usando {label}.'
  },
  browser_run: {
    script_missing: 'No se indicó un script de walkthrough.',
    script_missing_hint: 'Pasa --script=<walkthrough.json> (gramática de pasos en .aioson/docs/qa/browser-walkthrough.md).',
    script_invalid: 'Script de walkthrough inválido: {errors}',
    failed: 'El walkthrough no pudo ejecutarse: {error} {detail}',
    hint: '  Pista: {hint}',
    header: 'Walkthrough "{name}" -> {target} [{browser}]',
    step_ok: '  ok    #{index} {action}: {detail} ({ms}ms)',
    step_fail: '  FALLO #{index} {action}: {error}',
    failure_snapshot: '  Página en el momento del fallo (aria snapshot):',
    ids_title: 'Ids probados:',
    id_line: '  {id}: {status} (pasos {steps})',
    warning: 'Aviso: {warning}',
    console_summary: 'Consola: {errors} errores, {page_errors} errores de página, {warnings} avisos.',
    verdict_pass: 'Walkthrough PASS: {passed}/{total} pasos ok.',
    verdict_fail: 'Walkthrough FAIL: {passed}/{total} pasos ok ({planned} planificados).',
    report_written: 'Informe escrito: {path}',
    superseded: 'Eliminados {files} artefacto(s) de la ejecución anterior ({kb} KB) — la carpeta de artefactos guarda solo esta ejecución.',
    replay: 'Replay: {command}'
  },
  browser_snapshot: {
    target_missing: 'No se indicó una página.',
    target_missing_hint: 'Pasa --url=<url-pagina> o --file=<html>.',
    failed: 'El snapshot no pudo ejecutarse: {error} {detail}',
    header: 'Snapshot de {url} - "{title}" [{browser}]',
    login_wall: 'Cayó en una pantalla de login: conecta al navegador ya autenticado con --cdp en vez de scriptar credenciales.',
    empty: '(árbol de accesibilidad vacío)',
    truncated: '... {shown} de {total} líneas mostradas (sube --max-lines para ver más).'
  },
  qa_init: {
  ...
  qa_run: {
    browser_unavailable: 'Navegador no disponible ({error}). {hint}',
  ...
  pentester_report: {
    ...
    written: 'Informes HTML del Pentester escritos en: {path}',
    coverage_incomplete: 'La cobertura del Pentester sigue incompleta ({count} brecha(s)); el informe registra las limitaciones.'
  },
  web_map: {
  ...
  scan_project: {
    ...
    folder_required_example_cli:
      '  Sin API LLM     : aioson scan:project . --folder=src  -> entrega el mapa a @planner con un PRD activo',
    folder_required_example_prompt:
      '  Prompt listo    : aioson agent:prompt planner --tool=codex',
    ...
    spec_found: '  Legado   : spec.md encontrado — contexto historico opcional, sin autoridad de entrega',
    ...
    architecture_note: '  Nota     : los artefactos del escaneo son evidencia del repositorio; no crean una etapa o documento obligatorio.',
    ...
    local_cli_step_analyst: '  2. Con un PRD activo, ejecuta @planner — usa los mapas del escaneo como evidencia para el unico plan',
    local_cli_step_prompt_codex: '  3. Si el cliente no entiende @planner, genera un prompt listo: aioson agent:prompt planner --tool=codex',
    ...
    local_workflow_title: '\n\x1b[33m  Despues del mapa del repositorio:\x1b[0m',
    local_step_architect: '  Siguiente: ejecuta @planner con el PRD activo — escribe el unico plan ejecutable',
    local_step_dev: '  Despues: ejecuta @dev y @qa — los especialistas opcionales responden solo una pregunta concreta',
    next_steps: '\n  Proximos pasos:',
    step_analyst: '  1. Con un PRD activo, ejecuta @planner — usa discovery.md + skeleton-system.md como evidencia para un plan',
    step_architect: '  2. Ejecuta @dev — implementa el plan por la entrada normal de produccion',
    step_dev: '  3. Ejecuta @qa — realiza revision proporcional y un smoke por el camino real'
  },
  squad_investigate: {
  ...
  squadEval: {
    missing_slug: 'squad:eval requiere --squad=<slug>.',
    invalid_slug: 'Slug de squad no válido: {slug}.',
    manifest_missing: 'No se encontró el manifiesto del squad: {slug}.',
    result: 'Evaluación del squad {slug}: {verdict}',
    report: 'Informe: {path}'
  },
  ...
  system: {
    ...
    publish_protected_ts: '  TypeScript de runtime protegido: {count} archivo(s) (tipos eliminados + mangling, mismas rutas)',
