...
module.exports = {
  briefing_gate: {
    ...
    craft_below_bar: 'O briefing "{slug}" não pode ser aprovado: o protótipo mede abaixo da régua premium (prototype_visual_craft_below_bar).',
    craft_below_bar_detail: '  {detail}',
    craft_below_bar_fix: '  Reconstrua a partir das referências do dono (reference-identity-extract) ou na régua premium do registro, meça de novo com verify:artifact --kind=visual --runtime e aprove novamente — ou registre a decisão com --accept-craft.',
  ...
  cli: {
    ...
    help_update:
      'aioson update [path] [--dry-run] [--allow-downgrade] [--lang=<bcp47-tag>] [--locale=pt-BR]',
    ...
    rule_new: {
      ...
      name_required: 'rule:new exige --name=<nome-kebab-case>.',
      invalid_name: 'O nome da regra "{name}" precisa ser kebab-case (letras, digitos, hifens simples).',
      not_project: 'Nao ha diretorio .aioson/ aqui - execute dentro de um projeto AIOSON.',
      exists: 'A regra "{name}" ja existe em {path}. Edite o arquivo ou use --force para substituir.',
      invalid_load_tier: 'load_tier precisa ser `always` ou `trigger`.',
      invalid_priority: 'priority precisa ser um numero entre 0 e 100.',
      no_routing_dimension: 'Aviso: nenhum agents, triggers, task-types ou paths foi declarado, entao o context:select dificilmente alcancara esta regra. Declare ao menos um, ou use --load-tier=always.',
      next: 'Proximo passo: troque os enunciados de exemplo por requisitos concretos e verificaveis, depois rode aioson verify:artifact . --kind=rule --file={path}',
      error: 'rule:new falhou ({reason}).'
    },
    help_briefing_sources:
      'aioson briefing:sources [caminho] [--slug=<slug>] [--json] [--locale=pt-BR]',
    help_briefing_approve:
      'aioson briefing:approve [caminho] [--slug=<slug>] [--accept-craft] [--locale=pt-BR]',
    help_briefing_unapprove:
      'aioson briefing:unapprove [caminho] [--slug=<slug>] [--locale=pt-BR]',
    ...
    briefing_sources: {
      error: 'Não foi possível inspecionar as fontes do Briefing ({error}) para {slug}.',
      inspected: 'Pacote de fontes "{slug}" inspecionado: {files} arquivo(s), SQL={sql}, bloqueados={blocked}.',
      none: 'Nenhum pacote de fontes do Briefing foi encontrado em plans/.',
      discovered: 'Foram encontrados {packs} pacote(s) de fontes e {loose} arquivo(s) avulso(s).'
    },
    ...
    help_context_select:
      'aioson context:select [path] [--explain=<path[,path2]>] [--agent=<agente>] [--mode=planning|executing] [--task=<texto>] [--paths=<caminho[,caminho2]>] [--feature=<slug>] [--no-semantic] [--json] [--locale=pt-BR]',
    help_context_brief:
    ...
    help_exploration:
      'aioson exploration:<init|configure|references|intake|scan|add-run|record|validate|status|review|select|promote|run> [caminho] --slug=<slug> [opções] [--json]',
    help_chain_list:
      'aioson chain:list [caminho] [--feature=<slug>] [--agent=dev] [--status=<open,claimed>] [--include-resolved] [--limit=N] [--json] [--locale=pt-BR]',
    help_chain_claim:
      'aioson chain:claim [caminho] --agent=<agente> [--id=NC-N] [--feature=<slug>] [--limit=N] [--lease-minutes=N] [--json] [--locale=pt-BR]',
    help_chain_resolve:
      'aioson chain:resolve [caminho] --id=NC-N --agent=<agente> --token=<token> --outcome=<fixed|verified-no-change|false-positive|obsolete> --evidence=<texto> [--json] [--locale=pt-BR]',
    help_chain_release:
      'aioson chain:release [caminho] --id=NC-N --agent=<agente> --token=<token> [--json] [--locale=pt-BR]',
    help_chain_reconcile:
      'aioson chain:reconcile [caminho] [--json] [--locale=pt-BR]',
    chain_audit: {
    ...
    help_locale_diff: 'aioson locale:diff [agent] [--lang=<bcp47-tag>] [--json] [--locale=pt-BR]',
    help_test_agents: 'aioson test:agents [caminho] [--json] [--locale=pt-BR]',
    help_test_smoke:
    ...
    help_workflow_mode:
      'aioson workflow:mode [caminho] [--feature=<slug>] [--auto] [--step] [--json] [--locale=pt-BR]',
    ...
    help_review_prepare:
      'aioson review:prepare [caminho] --agent=<agente> --feature=<slug> [--artifact=<caminho>] [--json] [--locale=pt-BR]',
    help_review_check:
      'aioson review:check [caminho] --agent=<agente> --feature=<slug> --report=<caminho> [--json] [--locale=pt-BR]',
    help_review_status:
      'aioson review:status [caminho] --feature=<slug> [--json] [--locale=pt-BR]',
    help_parallel_init:
    ...
    help_qa_run:
      'aioson qa:run [path] [--url=<app-url>] [--feature=<slug>] [--persona=naive|hacker|power|mobile] [--headed] [--html] [--json] [--locale=pt-BR]',
    help_qa_scan:
      'aioson qa:scan [path] [--url=<app-url>] [--depth=3] [--max-pages=50] [--headed] [--html] [--json] [--locale=pt-BR]',
    help_browser_run:
      'aioson browser:run [path] --script=<walkthrough.json> [--url=<url-do-app>|--file=<html>] [--slug=<feature>] [--prototype] [--cdp=<endpoint>] [--browser=chrome|msedge|chromium] [--headed] [--continue] [--out=<dir>] [--no-persist] [--json] [--locale=pt-BR]',
    help_browser_snapshot:
      'aioson browser:snapshot [path] --url=<url-da-pagina>|--file=<html> [--target=<locator>] [--max-lines=80] [--cdp=<endpoint>] [--browser=chrome|msedge|chromium] [--headed] [--json] [--locale=pt-BR]',
    help_qa_report:
      'aioson qa:report [path] [--html] [--json] [--locale=pt-BR]',
    help_pentester_report:
      'aioson pentester:report [caminho] [--feature=<slug>] [--list] [--json] [--locale=pt-BR]',
    help_pentester_coverage:
      'aioson pentester:coverage [caminho] --feature=<slug> [--json] [--locale=pt-BR]',
    help_feature_list:
      'aioson feature:list [caminho] [--status=<status[,status2]>] [--limit=N] [--json] [--locale=pt-BR]',
    help_feature_current:
      'aioson feature:current [caminho] [--with-summary] [--json] [--locale=pt-BR]',
    help_feature_trace:
      'aioson feature:trace [caminho] --feature=<slug> [--json] [--locale=pt-BR]',
    help_feature_diff:
      'aioson feature:diff [caminho] --feature=<slug> [--base=<ref>] [--paths-only] [--json] [--locale=pt-BR]',
    help_setup_detect:
    ...
    help_harness_apply_validation:
      'aioson harness:apply-validation [path] --slug=<slug> [--input=<arquivo>] [--json] [--locale=pt-BR]',
    help_harness_approve:
      'aioson harness:approve [path] --slug=<slug> --gate=<gate-id> [--json] [--locale=pt-BR]',
    help_harness_reject:
      'aioson harness:reject [path] --slug=<slug> --gate=<gate-id> [--reason=<texto>] [--json] [--locale=pt-BR]',
    help_harness_status:
      'aioson harness:status [path] [--slug=<slug>] [--json] [--locale=pt-BR]',
    help_feature_close:
      'aioson feature:close [path] --feature=<slug> --verdict=PASS|FAIL [--residual=<texto>] [--preflight|--explain] [--force] [--no-archive] [--no-distill] [--json] [--locale=pt-BR]',
    help_feature_archive:
      'aioson feature:archive [path] --feature=<slug> [--dry-run] [--restore] [--sweep] [--force] [--json] [--locale=pt-BR]',
    help_gate_check:
      'aioson gate:check [path] --feature=<slug> --gate=A|B|C|D [--json] [--locale=pt-BR]',
    help_gate_approve:
      'aioson gate:approve [path] --feature=<slug> --gate=A|B|C|D [--json] [--locale=pt-BR]',
    help_prototype_check:
      'aioson prototype:check [path] --feature=<slug> [--json] [--locale=pt-BR]',
    help_ac_test_audit:
      'aioson ac:test-audit [path] --feature=<slug> [--strict] [--seed] [--json] [--locale=pt-BR]',
    help_preflight:
      'aioson preflight [path] [--agent=<nome>] [--feature=<slug>] [--json] [--locale=pt-BR]',
    help_pulse_update:
      'aioson pulse:update [path] --agent=<nome> [--feature=<slug>] [--gate=<texto>] [--verdict=<texto>] [--locale=pt-BR]',
    help_agent_done:
      'aioson agent:done [path] --agent=<nome> --summary=<texto> [--title=<texto>] [--status=completed|failed] [--json] [--locale=pt-BR]',
    help_dossier_add_finding:
      'aioson dossier:add-finding [path] --slug=<slug> --agent=<nome> --section=<nome> --content=<texto> [--json] [--locale=pt-BR]',
    help_detect_test_runner:
      'aioson detect:test-runner [path] [--json] [--locale=pt-BR]',
    help_dev_resume_data:
      'aioson dev:resume-data [path] [--json] [--locale=pt-BR]',
    ...
    help_delegation_plan:
      'aioson delegation:plan [path] --explicit-model-request --host=<host> [--provider=<provider>] --model=<modelo> --kind=<tipo> (--task=<texto>|--task-file=<caminho>) [--json]',
    help_delegation_run:
      'aioson delegation:run [path] --explicit-model-request --host=<host> [--provider=<provider>] --model=<modelo> --kind=<tipo> (--task=<texto>|--task-file=<caminho>) [--json]',
    harnessRetro: {
    ...
    help_genome_approve:
      'aioson genome:approve [path] --squad=<slug> --genome=<slug> [--executor=<slug>] [--specimen=<path>] [--json] [--locale=pt-BR]',
    help_genome_migrate:
    ...
    help_execution_run:
      'aioson execution:run [path] --feature=<slug> [--preflight] [--resume] [--fresh] [--wave=<n>] [--json] [--locale=pt-BR]',
    ...
    help_execution_status:
      'aioson execution:status [path] --feature=<slug> [--json] [--locale=pt-BR]',
  ...
  init: {
    usage_error:
      'Uso: aioson init <project-name> [--force] [--dry-run] [--all] [--no-hooks] [--lang=<bcp47-tag>] [--tool=codex|claude|opencode] [--locale=pt-BR]',
  ...
  hooks_setup: {
    installing: 'Instalando hooks do AIOSON (context:guard + telemetria de runtime) — desative com --no-hooks:',
    skipped: 'Hooks ignorados (--no-hooks). Instale depois com: aioson hooks:install',
    failed: 'Falha ao instalar hooks (nao bloqueia): {message}. Instale depois com: aioson hooks:install'
  },
  init_all: {
  ...
  update: {
    ...
    downgrade_blocked: 'Este projeto foi atualizado pela ultima vez com o AIOSON {projectVersion}, mas este CLI e {cliVersion}. Atualizar agora faria downgrade dos arquivos gerenciados. Atualize o CLI primeiro (npm install -g @jaimevalasek/aioson) e rode o update de novo, ou use --allow-downgrade para prosseguir mesmo assim.',
  ...
  doctor: {
    ...
    context_language_mismatch:
      '`interaction_language` ({interaction}) difere de `conversation_language` ({conversation})',
    ...
    retired_skill_trees: 'Arvores de skills aposentadas do framework ainda instaladas ({count})',
    retired_skill_trees_hint: 'Uma versao antiga copiou {paths} — um segundo engine de design e as specs de componentes do dashboard do proprio framework. Nada as roteia e o update nunca remove arquivos locais do projeto; apague-as (o engine interface-design e a unica skill de design embarcada) ou mantenha so o que este projeto realmente usa.',
    retired_design_preset: 'design_skill aponta para um preset visual fixo aposentado ({id})',
    retired_design_preset_hint_local: "O framework nao embarca mais presets visuais fixos — todo projeto que escolhia o mesmo ficava igual. {path} e uma copia local instalada por uma versao antiga; continua funcionando enquanto design_skill a nomear. Rota recomendada: design_skill 'interface-design' (o engine) com as referencias do dono extraidas em identity.md (.aioson/docs/reference-identity.md).",
    retired_design_preset_hint_missing: "O framework nao embarca mais presets visuais fixos e nao existe copia em .aioson/skills/design/{id}/ nem em .aioson/installed-skills/{id}/ — os produtores visuais caem no engine interface-design. Defina design_skill como 'interface-design' (com identity.md a partir das referencias do dono) ou deixe em branco de proposito.",
    retired_design_preset_profile: 'O perfil de instalacao salvo ainda seleciona presets visuais aposentados ({ids}) — nada os embarca; o engine interface-design e instalado em todo perfil. `aioson install . --reconfigure` regrava o perfil sem a pergunta de design.',
  ...
  locale_apply: {
    ...
    context_updated: 'Campos de idioma do project.context.md sincronizados: {locale}',
    context_would_update: '[dry-run] Campos de idioma do project.context.md seriam sincronizados: {locale}',
    context_unchanged: 'Campos de idioma do project.context.md ja estavam sincronizados: {locale}',
    context_missing: 'project.context.md nao encontrado; a preferencia de idioma nao foi persistida.',
    context_invalid: 'project.context.md tem frontmatter invalido; a preferencia de idioma nao foi alterada.'
  },
  smoke: {
  ...
  qa_doctor: {
    ...
    browser_cdp_ok: 'Endpoint CDP alcançável ({endpoint}): {browser}.',
    browser_cdp_unreachable: 'Endpoint CDP não alcançável ({endpoint}).',
    browser_cdp_hint: 'Inicie o browser com --remote-debugging-port=9222 (com --user-data-dir dedicado) ou habilite em chrome://inspect/#remote-debugging.',
    browser_chrome_ok: 'Google Chrome instalado ({binary}) - utilizável com --browser=chrome.',
    browser_chrome_missing: 'Google Chrome não detectado.',
    browser_edge_ok: 'Microsoft Edge instalado ({binary}) - utilizável com --browser=msedge.',
    browser_edge_missing: 'Microsoft Edge não detectado.',
    browser_preferred: 'Browser padrão para qa:run / browser:run: {label}.',
    browser_none: 'Nenhum browser disponível: sem Chromium empacotado, sem Chrome/Edge instalado, sem endpoint CDP.',
    browser_none_hint: 'Instale o Google Chrome, rode `npx playwright install chromium` ou conecte com --cdp=http://127.0.0.1:9222.',
    chromium_missing_fallback: 'Chromium empacotado não encontrado - usando {label}.'
  },
  browser_run: {
    script_missing: 'Nenhum script de walkthrough informado.',
    script_missing_hint: 'Passe --script=<walkthrough.json> (gramática dos passos em .aioson/docs/qa/browser-walkthrough.md).',
    script_invalid: 'Script de walkthrough inválido: {errors}',
    failed: 'O walkthrough não pôde rodar: {error} {detail}',
    hint: '  Dica: {hint}',
    header: 'Walkthrough "{name}" -> {target} [{browser}]',
    step_ok: '  ok    #{index} {action}: {detail} ({ms}ms)',
    step_fail: '  FALHA #{index} {action}: {error}',
    failure_snapshot: '  Página no momento da falha (aria snapshot):',
    ids_title: 'Ids provados:',
    id_line: '  {id}: {status} (passos {steps})',
    warning: 'Aviso: {warning}',
    console_summary: 'Console: {errors} erros, {page_errors} erros de página, {warnings} avisos.',
    verdict_pass: 'Walkthrough PASS: {passed}/{total} passos ok.',
    verdict_fail: 'Walkthrough FAIL: {passed}/{total} passos ok ({planned} planejados).',
    report_written: 'Relatório gravado: {path}',
    superseded: 'Removidos {files} artefato(s) da rodada anterior ({kb} KB) — a pasta de artefatos guarda só esta rodada.',
    replay: 'Replay: {command}'
  },
  browser_snapshot: {
    target_missing: 'Nenhuma página informada.',
    target_missing_hint: 'Passe --url=<url-da-pagina> ou --file=<html>.',
    failed: 'O snapshot não pôde rodar: {error} {detail}',
    header: 'Snapshot de {url} - "{title}" [{browser}]',
    login_wall: 'Caiu numa tela de login: conecte ao browser já autenticado com --cdp em vez de scriptar credenciais.',
    empty: '(árvore de acessibilidade vazia)',
    truncated: '... {shown} de {total} linhas mostradas (aumente --max-lines para ver mais).'
  },
  qa_init: {
  ...
  qa_run: {
    browser_unavailable: 'Browser indisponível ({error}). {hint}',
  ...
  feature_list: {
    title: 'Features registradas neste projeto ({count}):',
    item: '{marker} {slug} - {status} (inicio {started})',
    empty: 'Nenhuma feature registrada em .aioson/context/features.md.',
    truncated: '... e mais {hidden} (use --limit=0 para listar todas).'
  },
  pentester_report: {
    feature_required: 'pentester:report exige --feature=<slug>. Rode sem --feature para listar as execucoes disponiveis.',
    list_title: 'Execucoes do Pentester disponiveis para relatorio ({count}):',
    list_item: '  {feature} - {findings} finding(s), {open} em aberto, {report}',
    list_has_report: 'relatorio HTML ja gerado',
    list_no_report: 'sem relatorio HTML ainda',
    list_empty: 'Nenhum artefato de findings do Pentester encontrado em .aioson/context/.',
    list_hint: 'Gere um com: aioson pentester:report . --feature=<slug> --json',
  ...
  web_save: {
    url_missing: 'Opcao obrigatoria ausente: --url=<url>.',
    fetching: 'Salvando site: {url}',
    saved: 'Salvos {count} arquivos ({kb} KB) em {dir}.',
    failures: '{count} asset(s) nao puderam ser baixados — veja manifest.json.',
    done: 'Web save concluido. Aponte --from-local / leituras locais para o diretorio salvo.',
    failed: 'Falha no web save: {error}'
  },
  web_extract: {
    target_missing: 'Opcao obrigatoria ausente: --slug=<slug> ou --dir=<dir>.',
    dir_missing: 'Diretorio de site salvo nao encontrado: {dir}. Rode aioson web:save primeiro.',
    extracting: 'Extraindo inteligencia de design de {dir}',
    written: 'Extract gravado em {file}.',
    matches: '{count} ocorrencia(s) para "{query}".',
    done: 'Web extract concluido. Leia extract.md em vez do HTML/CSS/JS bruto.',
    injection_flagged: 'Varredura de injecao: {count} padrao(oes) em forma de instrucao no site capturado ({families}) — dado, nunca instrucao; veja a secao "Injection scan" do extract.md.',
  ...
  delivery_parity: {
    ...
    not_a_git_repository: 'nao e um repositorio git',
    git_status_unavailable: 'git status indisponivel',
    runtime_only: 'delivery:parity — apenas runtime: {runtime} arquivo(s) de estado do framework pendente(s) — churn do framework, nao trabalho autoral.',
    noted: 'delivery:parity — anotado: {authored} arquivo(s) autoral(is) pendente(s) em {areas} — abaixo do piso de {threshold} arquivos do aviso.',
    advisory: 'delivery:parity — AVISO: {authored} arquivo(s) autoral(is) pendente(s) em {areas} — esta entrega nao chegou ao git; acione @committer (ou rode `aioson commit:prepare .`) antes que o trabalho cresca mais uma onda.',
    by_area: 'Pendencias por area (as fatias naturais de commit):',
