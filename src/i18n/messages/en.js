...
module.exports = {
  ...
  cli: {
    ...
    help_item_line: '  {text}',
    help_init:
      'aioson init <project-name> [--force] [--dry-run] [--all] [--lang=<bcp47-tag>] [--tool=codex|claude|opencode] [--locale=en]',
    help_install:
    ...
    help_evidence_prune: 'aioson evidence:prune [path] [--slug=<feature>] [--all] [--dry-run] [--json] [--locale=en]',
    ...
    help_context_usage:
      'aioson context:usage [path] [--since=<days>] [--feature=<slug>] [--json] [--locale=en]',
    help_context_load:
    ...
    help_test_agents: 'aioson test:agents [path] [--json] [--locale=en]',
    help_test_smoke:
    ...
    help_review_prepare:
      'aioson review:prepare [path] --agent=<agent> --feature=<slug> [--artifact=<path>] [--json] [--locale=en]',
    help_review_check:
      'aioson review:check [path] --agent=<agent> --feature=<slug> --report=<path> [--json] [--locale=en]',
    help_review_status:
      'aioson review:status [path] --feature=<slug> [--json] [--locale=en]',
    help_parallel_init:
    ...
    help_mcp_init:
      'aioson mcp:init [path] [--tool=claude|codex|opencode] [--dry-run] [--json] [--locale=en]',
    help_mcp_doctor:
    ...
    help_qa_run:
      'aioson qa:run [path] [--url=<app-url>] [--feature=<slug>] [--persona=naive|hacker|power|mobile] [--headed] [--html] [--json] [--locale=en]',
    help_qa_scan:
    ...
    help_profiler_coverage:
      'aioson profiler:coverage [path] --slug=<slug> [--file=<path>] [--json] [--locale=en]',
    help_harness_init:
    ...
    help_squad_validate:
      'aioson squad:validate [path] --squad=<slug> [--strict] [--json] [--locale=en]',
    help_squad_eval:
      'aioson squad:eval [path] --squad=<slug> [--json] [--locale=en]',
    help_squad_pilot_approve:
      'aioson squad:pilot-approve [path] --squad=<slug> [--json] [--locale=en]',
    help_squad_export:
    ...
    help_runtime_storage:
      'aioson runtime:storage [path] [--older-than=<days>] [--output-older-than=<days>] [--json] [--locale=en]',
    help_runtime_prune:
      'aioson runtime:prune [path] [--older-than=<days>] [--output-older-than=<days>] [--dry-run] [--compact] [--force] [--json] [--locale=en]',
    help_runtime_compact:
      'aioson runtime:compact [path] [--force] [--json] [--locale=en]',
    help_agent_recover:
  ...
  hooks_setup: {
    installing: 'Installing AIOSON hooks (context:guard + runtime telemetry) — opt out with --no-hooks:',
    skipped: 'Hooks skipped (--no-hooks). Install later with: aioson hooks:install',
    failed: 'Hooks install failed (non-blocking): {message}. Install later with: aioson hooks:install'
  },
  init_all: {
  ...
  doctor: {
    ...
    retired_design_doc_seed: 'Retired design-doc seed in .aioson/context/design-doc.md ({kind})',
    retired_design_doc_seed_hint_verbatim: '{path} is the code-organization seed older installers copied verbatim — the framework\'s own Node.js CLI layout, not this project\'s. Nothing produces or requires it; structural governance lives in .aioson/design-docs/. Delete it (`aioson doctor . --fix` removes a verbatim copy).',
    retired_design_doc_seed_hint_derived: '{path} still carries the retired seed\'s header ("must be loaded before any implementation", "generated by @discovery-design-doc") and its Node.js CLI layout. Keep only what this project decided, as a named technical decision record, or delete the file; structural governance lives in .aioson/design-docs/.',
    fix_action_retired_design_doc_seed: 'Delete .aioson/context/design-doc.md when it is the retired seed copied verbatim (edited copies are kept for review)',
    retired_skill_trees: 'Retired framework skill trees still installed ({count})',
    retired_skill_trees_hint: 'An older version copied {paths} — a second design engine and the framework\u2019s own dashboard component specs. Nothing routes to them and update never removes project-local files; delete them (the interface-design engine is the only shipped design skill), or keep what this project actually uses.',
    retired_design_preset: 'design_skill names a retired fixed design preset ({id})',
    retired_design_preset_hint_local: "The framework no longer ships fixed design presets — every project that picked the same one looked the same. {path} is a project-local copy an older version installed; it keeps working while design_skill names it. Recommended route: design_skill 'interface-design' (the engine) with the owner's references extracted into identity.md (.aioson/docs/reference-identity.md).",
    retired_design_preset_hint_missing: "The framework no longer ships fixed design presets and no copy exists under .aioson/skills/design/{id}/ or .aioson/installed-skills/{id}/ — visual producers fall back to the interface-design engine. Set design_skill to 'interface-design' (with identity.md from the owner's references) or leave it blank on purpose.",
    retired_design_preset_profile: 'The saved install profile still selects retired design presets ({ids}) — nothing ships them; the interface-design engine is installed for every profile. `aioson install . --reconfigure` rewrites the profile without the design question.',
  ...
  agents: {
    ...
    none: 'none',
    prompt_usage_error:
      'Usage: aioson agent:prompt <agent> [path] [--tool=codex|claude|opencode] [--lang=en|pt-BR|es|fr] [--locale=en]',
    ...
    help_opt_tool: 'Target tool (codex|claude|opencode)',
  ...
  locale_apply: {
    ...
    context_updated: 'project.context.md language fields synchronized: {locale}',
    context_would_update: '[dry-run] project.context.md language fields would be synchronized: {locale}',
    context_unchanged: 'project.context.md language fields already synchronized: {locale}',
    context_missing: 'project.context.md not found; language preference was not persisted.',
    context_invalid: 'project.context.md has invalid frontmatter; language preference was not changed.'
  },
  smoke: {
  ...
  mcp_doctor: {
    ...
    presets_coverage_full: 'All tool presets are present (claude, codex, opencode).',
  ...
  browser_run: {
    ...
    superseded: 'Cleared {files} artifact(s) from the previous run ({kb} KB) — the artifact folder holds this run only.',
  ...
  feature_list: {
    title: 'Features registered in this project ({count}):',
    item: '{marker} {slug} - {status} (started {started})',
    empty: 'No features registered yet in .aioson/context/features.md.',
    truncated: '... and {hidden} more (use --limit=0 to list all).'
  },
  pentester_report: {
    feature_required: 'pentester:report requires --feature=<slug>. Run it without --feature to list the available runs.',
    list_title: 'Pentester runs available for reporting ({count}):',
    list_item: '  {feature} - {findings} finding(s), {open} open, {report}',
    list_has_report: 'HTML report already generated',
    list_no_report: 'no HTML report yet',
    list_empty: 'No Pentester findings artifact found in .aioson/context/.',
    list_hint: 'Generate one with: aioson pentester:report . --feature=<slug> --json',
  ...
  scan_project: {
    ...
    folder_required_example_cli:
      '  No API in aioson: aioson scan:project . --folder=src  -> then give the map to @planner for an active PRD',
    folder_required_example_prompt:
      '  Ready prompt    : aioson agent:prompt planner --tool=codex',
    ...
    spec_found: '  Legacy   : spec.md found — optional historical context, not delivery authority',
    ...
    architecture_note: '  Note     : scan artifacts are repository evidence; they do not add a required agent stage or document.',
    ...
    local_cli_step_analyst: '  2. With an active PRD, run @planner — it uses scan-index.md + scan-folders.md + scan-<folder>.md as repository evidence for the one implementation plan',
    local_cli_step_prompt_codex: '  3. If the client does not understand @planner, generate a ready prompt: aioson agent:prompt planner --tool=codex',
    local_cli_step_prompt_claude: '  4. Swap --tool=codex for --tool=claude when needed',
    local_cli_step_model_hint: '  5. If your client supports model selection, prefer a fast/cheap model for this discovery step',
    local_workflow_title: '\n\x1b[33m  After the repository map:\x1b[0m',
    local_step_architect: '  Next: run @planner with the active PRD — it writes the single executable plan',
    local_step_dev: '  Then: run @dev and @qa — optional specialists answer only a concrete question',
    next_steps: '\n  Next steps:',
    step_analyst: '  1. With an active PRD, run @planner — use discovery.md + skeleton-system.md as repository evidence for one executable plan',
    step_architect: '  2. Run @dev — implement that plan through the normal production entry point',
    step_dev: '  3. Run @qa — perform proportional acceptance review and one production-path smoke'
  },
  squad_investigate: {
  ...
  squadEval: {
    missing_slug: 'squad:eval requires --squad=<slug>.',
    invalid_slug: 'Invalid squad slug: {slug}.',
    manifest_missing: 'Squad manifest not found: {slug}.',
    result: 'Squad eval {slug}: {verdict}',
    report: 'Report: {path}'
  },
