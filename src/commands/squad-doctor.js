...
async function resolveSquadSlug(targetDir, requestedSlug) {
  ...
  const metadataDir = path.join(targetDir, '.aios-lite', 'squads');
...
async function parseSquadPaths(targetDir, slug) {
  ...
  return {
    ...
    agentsDir: hasPackageDir
      ? path.join(targetDir, normalizeRel(packageInfo.agentsDir || `.aios-lite/squads/${slug}/agents`))
