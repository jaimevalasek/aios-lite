...
function extractFromHtml(htmlText, sourceLabel, acc) {
  ...
  if (!acc.title) {
    acc.title = stripHtmlTags(source.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] || '').slice(0, 160);
  }
  if (!acc.description) {
    acc.description = (source.match(/<meta[^>]+(?:name|property)=["'](?:og:)?description["'][^>]+content=["']([^"']+)["']/i)?.[1] || '').slice(0, 240);
  }
  ...
  while ((match = landmarkPattern.exec(source)) !== null && acc.topology.length < CAPS.topology) {
    ...
    const heading = stripHtmlTags(rest.match(/<h[1-3][^>]*>([\s\S]*?)<\/h[1-3]>/i)?.[1] || '').slice(0, CAPS.headingChars);
  ...
  for (const file of files) {
    ...
    for (let index = 0; index < lines.length && matches.length < cap; index++) {
      ...
      matches.push({
        ...
        before: lines.slice(Math.max(0, index - bounded), index).map((entry) => entry.trim().slice(0, 300)),
        after: lines.slice(index + 1, index + 1 + bounded).map((entry) => entry.trim().slice(0, 300))
      });
    }
  }
