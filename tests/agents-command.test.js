...
test('agent:prompt enforces workflow and routes to the active stage', async () => {
  ...
  const runtime = await openRuntimeDb(dir, { mustExist: true });
  try {
    const stageRun = runtime.db.prepare("SELECT agent_name, source, workflow_stage, status FROM agent_runs WHERE agent_name = '@product' ORDER BY updated_at DESC LIMIT 1").get();
    ...
    assert.equal(stageRun.agent_name, '@product');
    assert.equal(stageRun.source, 'workflow');
    assert.equal(stageRun.workflow_stage, 'product');
