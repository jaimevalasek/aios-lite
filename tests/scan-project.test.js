...
test('scan:project requires --folder before scanning', async () => {
  const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aios-forge-scan-project-required-folder-'));
...
test('scan:project explains missing provider API key with direct config guidance', async () => {
  const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aios-forge-scan-project-'));
  const originalExitCode = process.exitCode;
  process.exitCode = undefined;

  try {
    await fs.writeFile(
      path.join(projectDir, 'aios-forge-models.json'),
      `${JSON.stringify({
    ...
    assert.equal(
      logger.lines.some((line) =>
        line.includes('A chave de API do provider "deepseek" ainda nao foi configurada em aios-forge-models.json')
      ),
      true
    );
    assert.equal(
...
test('scan:project runs in local-only mode by default and writes folder-specific scan files', async () => {
  const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aios-forge-scan-project-local-'));
  const originalExitCode = process.exitCode;
  process.exitCode = undefined;
  ...
  try {
    ...
    await fs.mkdir(path.join(projectDir, '.aios-forge', 'agents'), { recursive: true });
    await fs.mkdir(path.join(projectDir, '.aios-forge', 'context'), { recursive: true });
    await fs.mkdir(path.join(projectDir, '.aios-forge', 'squads', 'custom-squad'), { recursive: true });
    await fs.mkdir(path.join(projectDir, '.aios-forge', 'genomas'), { recursive: true });
    await fs.writeFile(path.join(projectDir, '.aios-forge', 'agents', 'setup.md'), '# managed\n', 'utf8');
    await fs.writeFile(path.join(projectDir, '.aios-forge', 'squads', 'memory.md'), '# managed memory\n', 'utf8');
    await fs.writeFile(path.join(projectDir, '.aios-forge', 'context', 'discovery.md'), '# discovery\n', 'utf8');
    await fs.writeFile(
      path.join(projectDir, '.aios-forge', 'squads', 'custom-squad', 'squad.manifest.json'),
      '{ "slug": "custom-squad" }\n',
      'utf8'
    );
    await fs.writeFile(path.join(projectDir, '.aios-forge', 'genomas', 'demo.md'), '# demo genome\n', 'utf8');
    ...
    const indexPath = path.join(projectDir, '.aios-forge/context/scan-index.md');
    const foldersPath = path.join(projectDir, '.aios-forge/context/scan-folders.md');
    const srcPath = path.join(projectDir, '.aios-forge/context/scan-src.md');
    const forgePath = path.join(projectDir, '.aios-forge/context/scan-aios-forge.md');
    ...
    assert.match(foldersContent, /\.aios-forge\//);
    ...
    assert.match(forgeContent, /# AIOS Forge Generated Map/);
...
test('scan:project builds a compact scan index with folder scans, optional summaries and sizes', () => {
  const markdownWithSummaries = buildScanIndexMarkdown({
    ...
    foldersPath: '/tmp/project/.aios-forge/context/scan-folders.md',
    folderScans: [{
      folder: 'src',
      relativePath: '.aios-forge/context/scan-src.md',
      absolutePath: '/tmp/project/.aios-forge/context/scan-src.md'
    }],
    forgePath: '/tmp/project/.aios-forge/context/scan-aios-forge.md',
  ...
  assert.match(markdownWithSummaries, /Folder `src\/`: `\/tmp\/project\/\.aios-forge\/context\/scan-src\.md`/);
  assert.match(markdownWithSummaries, /AIOS Forge generated entries: 2/);
  ...
  const markdownTitlesOnly = buildScanIndexMarkdown({
    ...
    folderScans: [{
      folder: 'app',
      relativePath: '.aios-forge/context/scan-app.md',
      absolutePath: '/tmp/project/.aios-forge/context/scan-app.md'
    }],
    forgePath: '/tmp/project/.aios-forge/context/scan-aios-forge.md',
...
test('scan:project prompt keeps raw file contents optional and includes requested folder scans', () => {
  const promptWithSummaries = buildPrompt({
    ...
    forgeMapMarkdown: '# AIOS Forge Generated Map\n\n## Squads\n```text\n.aios-forge/squads/\n```',
  ...
  assert.match(promptWithSummaries, /## AIOS Forge Generated Map/);
  assert.doesNotMatch(promptWithSummaries, /## Key Files/);
  assert.match(promptWithSummaries, /## Project Context \(aios-forge\)/);
  ...
  const promptWithRaw = buildPrompt({
    ...
    forgeMapMarkdown: '# AIOS Forge Generated Map\n\n_No generated AIOS Forge artifacts detected yet_',
    keyContents: { 'package.json': '{ "name": "demo" }' },
    projectContext: '',
    specContent: '',
    summaryMode: 'raw'
...
test('scan:project dry-run returns requested folder paths without writing files', async () => {
  const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aios-forge-scan-project-dry-run-'));
  const originalExitCode = process.exitCode;
  process.exitCode = undefined;
  ...
  try {
    ...
    assert.equal(
      ...
      path.join(projectDir, '.aios-forge/context/scan-index.md')
    );
    assert.equal(
      result.scanFoldersPath,
      path.join(projectDir, '.aios-forge/context/scan-folders.md')
    );
    assert.deepEqual(result.scanFolderPaths, [
      path.join(projectDir, '.aios-forge/context/scan-src.md'),
      path.join(projectDir, '.aios-forge/context/scan-app.md')
    ]);
    assert.equal(
      result.scanForgePath,
      path.join(projectDir, '.aios-forge/context/scan-aios-forge.md')
    );
    assert.equal(
