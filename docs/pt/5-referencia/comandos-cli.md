...
| `rules:check` | Verifica se o **código obedece** as regras (enquanto o `rules:lint` verifica se as regras estão bem formadas). Roda os verificadores determinísticos declarados por `enforcement:` no frontmatter de regras, docs e skills de processo. Aceita `--changed` (só o diff), `--paths`, `--rule`, `--strict`, `--json`. | Durante a implementação, a cada fase — é determinístico e barato de repetir. O motor também o dispara sozinho no fim do DEV/QA e no gate de estágio. Um `HIGH` é regra sendo quebrada: nenhum PRD, plano ou desvio de dossiê vence uma regra |
