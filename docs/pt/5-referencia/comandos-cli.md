...
| `execution:run` | Roda o plano compilado onda a onda: cada unidade de faixa é um pipeline `dev → qa` de processos externos efêmeros (dev implementa dentro dos arquivos da unidade no modo de escrita não assistido do host; qa revisa/testa com o perfil qa-lane e corrige no máximo `qa.max_fix_files` arquivos, medidos), até `parallel.max_concurrent_lanes` ao mesmo tempo; preflight determinístico (`--preflight`: plano fresco, manifesto, host na PATH e com flag não assistido no registro, sonda de escrita da assinatura não bloqueada — assinatura sem sonda é aviso); segura o lease da feature e espera o lease de um run morto expirar (`run_lease_held` só para lease que alguém ainda renova, com caminho do lock e tempo restante — nunca apague o lock à mão); orçamento por unidade `--unit-timeout=<ms>` > `execution.unit_timeout_ms` > 60 min, `0` = sem limite, e mudar o orçamento nunca invalida o plano; deriva, estagnação (`stalled`) e inatividade só no disco (`unproductive`) medidas; unidade que não roda ou reprova vira `decision_required` (estado + evento de telemetria; um `timeout` diz se ainda escrevia ou nunca escreveu) e o run pausa; `--resume` continua idempotente; linhas ao vivo no stdout (stderr com `--json`) | Pelo @orchestrator na fase DEV orquestrada. Aceita `--feature`, `--preflight`, `--resume`, `--fresh`, `--wave`, `--unit-timeout`, `--json` |
