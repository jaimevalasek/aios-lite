...
| `execution:offer` | Responde se a execução orquestrada (faixas compiladas em processos externos paralelos, um host/modelo por papel) está disponível: `.aioson/config/execution-roles.json` presente/válido/habilitado (escrito pelo cliente desktop, nunca pelo framework) e todo papel assinado; com `--feature` descreve as tabelas do plano e o estado compilado. Sempre sai com 0 | Pelo planner antes de oferecer a escolha; pelo cliente supervisor. Aceita `--feature`, `--json` |
